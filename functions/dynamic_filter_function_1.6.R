# Fixed Dynamic Protein Filtering System - Now includes NA values in filters

# UI Module - Same as before
...
# Server Module - FIXED to include NAs
dynamicFilterServer <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    # FIXED: filtered_data now includes NAs when selected
    filtered_data <- reactive({
      req(data())  # Wait for data to exist
      df <- data()
      ...
      for (i in 1:values$filter_count) {
        ...
        if (is.numeric(df[[column_name]])) {
          ...
          # FIXED: For character/factor - include NAs if "(NA)" is selected
          if ("(NA)" %in% filter_value) {
            # Keep rows where value is NA OR in the selected values
            df <- df[is.na(df[[column_name]]) | 
                       (!is.na(df[[column_name]]) & df[[column_name]] %in% filter_value), ]
          } else {
            # Only keep rows with selected values (exclude NAs)
    ...
    # FIXED: Generate value input - now includes "(NA)" as an option
    observe({
      req(data())
      req(nrow(data()) > 0)
      df <- data()
        ...
        local({
          ...
          output[[paste0("filter_value_", filter_num)]] <- renderUI({
            ...
            if (is.numeric(column_data)) {
              ...
              # FIXED: Add "(NA)" as a choice if there are any NAs
              has_na <- any(is.na(column_data))
              if (has_na) {
                unique_values <- c(unique_values, "(NA)")
              }
              
              # Use stored values or default to all (including NA if present)
              default_vals <- if (!is.null(stored_value)) {
                stored_value
              } else {
                unique_values  # Include all by default, including "(NA)"
              }
