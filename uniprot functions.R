...
# Enhanced UniProt search function that uses selected fields
get_uniprot_info_enhanced <- function(uniprot_id, selected_fields = default_selected_fields) {
  tryCatch({
    ...
    for (field in selected_fields) {
      field_value <- extract_uniprot_field(data, field)
      result_df[[field]] <- field_value
    }
    
    # Add derived fields for protein design
    if ("cc_subcellular_location" %in% selected_fields) {
      result_df$secreted <- determine_secreted_status(result_df$cc_subcellular_location)
    }
    
    if ("ft_transmem" %in% selected_fields) {
      result_df$membrane_protein <- ifelse(
        !is.na(result_df$ft_transmem) & result_df$ft_transmem != "Not available", "Y", "N"
      )
    }
    
    if ("length" %in% selected_fields) {
      result_df$very_large <- ifelse(
        !is.na(result_df$length) & result_df$length > 1000, "Y", "N"
      )
    }
