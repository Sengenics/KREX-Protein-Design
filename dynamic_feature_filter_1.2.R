  ...
  tagList(
    ...
    wellPanel(
      ...
      div(
        ...
        helpText("Default exclusions: Transit peptide, Propeptide, Peptide, Non-terminal residue, Signal"),
      ...
      # Critical structural features near termini
      div(
        h6("Exclude proteins with critical structural features near termini:"),
        checkboxInput(ns("exclude_features_near_termini"), 
                      "Exclude critical structural features near protein termini", 
                      value = TRUE),
        
        conditionalPanel(
          condition = paste0("input['", ns("exclude_features_near_termini"), "']"),
          
          fluidRow(
            column(4,
                   numericInput(ns("terminus_distance"), 
                                "Distance from terminus (amino acids):",
                                value = 50, min = 10, max = 200, step = 10)
            ),
            column(4,
                   selectInput(ns("terminus_type"), 
                               "Apply to terminus:",
                               choices = c("N-terminus only" = "N",
                                           "C-terminus only" = "C", 
                                           "Both termini" = "both"),
                               selected = "both")
            ),
            column(4,
                   uiOutput(ns("critical_features_selector"))
            )
          ),
          
          helpText("Removes proteins where important structural elements are too close to the ends, which can interfere with protein design/expression")
        )
      ),
...
featureExclusionServer <- function(id, protein_data, feature_data) {
  moduleServer(id, function(input, output, session) {
    ...
    # Critical structural features selector
    output$critical_features_selector <- renderUI({
      types <- feature_types()
      if (length(types) == 0) {
        return(div("No feature types available"))
      }
      
      # Default critical structural features
      critical_structural_features <- c("Domain", "Active site", "Binding site", 
                                        "Metal binding", "Disulfide bond",
                                        "Coiled coil", "Beta strand", "Helix")
      
      # Only select defaults that exist in the data
      selected_critical <- intersect(critical_structural_features, types)
      
      selectInput(ns("critical_feature_types"), 
                  "Critical structural features:",
                  choices = types,
                  multiple = TRUE,
                  selected = selected_critical)
    })
    output$feature_type_selector <- renderUI({
      ...
      # Default exclusions - match the exact feature names from the UI
      default_exclusions <- c("Transit peptide", "Propeptide", "Peptide", "Non-terminal residue", "Signal")
    ...
    filtered_proteins <- reactive({
      ...
      # 7. Exclude proteins with critical features near termini
      if (isTRUE(input$exclude_features_near_termini)) {
        distance_cutoff <- input$terminus_distance
        terminus_type <- input$terminus_type
        critical_features <- input$critical_feature_types
        
        if (!is.null(distance_cutoff) && !is.null(terminus_type) && 
            !is.null(critical_features) && length(critical_features) > 0) {
          
          # Get protein lengths for terminus calculations
          length_col <- NULL
          if ("length" %in% names(proteins)) {
            length_col <- "length"
          } else if ("Length" %in% names(proteins)) {
          ...
          if (!is.null(length_col)) {
            protein_lengths <- proteins %>%
              select(uniprot_id, protein_length = !!sym(length_col))
            
            # Find features too close to termini
            features_near_termini <- features %>%
              filter(type %in% critical_features) %>%
              ...
              mutate(
                ...
                dist_from_c = protein_length - end,
                # Check if too close based on terminus_type
                too_close = case_when(
                  terminus_type == "N" ~ dist_from_n <= distance_cutoff,
                  terminus_type == "C" ~ dist_from_c <= distance_cutoff,
                  terminus_type == "both" ~ (dist_from_n <= distance_cutoff) | (dist_from_c <= distance_cutoff),
                  TRUE ~ FALSE
                )
              ) %>%
              filter(too_close) %>%
            ...
            proteins_to_exclude <- c(proteins_to_exclude, features_near_termini)
          }
        }
      }
      
      # 8. Apply additional feature filters
      for (i in 1:values$additional_filter_count) {
    ...
    output$exclusion_summary <- renderText({
      ...
      if (isTRUE(input$exclude_features_near_termini)) {
        terminus_desc <- switch(input$terminus_type %||% "both",
                                "N" = "N-terminus",
                                "C" = "C-terminus", 
                                "both" = "both termini")
        exclusions <- c(exclusions, paste0("Critical features within ", 
                                           input$terminus_distance %||% 50, 
                                           " aa of ", terminus_desc))
      }
